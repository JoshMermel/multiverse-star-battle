...
def does_violates_subset_ambiguity(row):
    """
    Returns (violates, inconclusive): violates is True if the puzzle
    violates the property (i.e., there exists a subset of size M-1 that is
    NOT ambiguous / uniquely solvable). inconclusive is True if at least
    one subset's search timed out before it could confirm either way (see
    get_solutions_capped_multi's docstring) -- violates is meaningless
    when inconclusive is True; the caller should report it separately
    rather than trust a False as "confirmed fine".
    """
    n = int(row['N'])
    ...
    for subset in combinations(grids, m - 1):
        ...
        if joint_solutions is None:
            # Timed out before confirming either way -- don't report this
            # puzzle as passing OR failing; flag it as unverified instead.
            return False, True
...
def verify_csv_subset_ambiguity(csv_path):
    ...
    try:
        with open(csv_path, mode='r', newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                violates, inconclusive = does_violates_subset_ambiguity(row)
                if inconclusive:
                    inconclusive_count += 1
                    print(f" Inconclusive (timed out): Puzzle '{name}' (N={row['N']}) -- could not confirm either way.")
                elif violates:
                    invalid_count += 1
    ...
    print(f"Puzzles inconclusive (timed out): {inconclusive_count}")
    print("="*40)
